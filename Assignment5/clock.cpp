...
Clock::Clock(Cpu* cpu, Memory* memory, IMemory* imemory, IODevice* iodevice, int debug) {
...
Clock* Clock::getClock(Cpu* cpu, Memory* memory, IMemory* imemory, IODevice* iodevice, int debug) {
    // Singleton Method
    if (clk_instance == nullptr) {
        clk_instance = new Clock(cpu, memory, imemory, iodevice, debug);
    }
    return clk_instance;
}
