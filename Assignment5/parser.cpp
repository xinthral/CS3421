...
Parser::Parser(int debug) {
    ...
    _iodevice = new IODevice(DEBUG);
    _imemory = new IMemory(DEBUG);
    _cache = new Cache(_memory, DEBUG);
    _cpu = new Cpu(_cache, _memory, _imemory, DEBUG);
    _clock = _clock->getClock(_cpu, _memory, _imemory, _iodevice, DEBUG);

    // Devices Options
    const int dev_num = 6;
    char* device_objects[dev_num] = {"cache", "clock", "cpu", "memory", "imemory", "iodev"};
        ...
        while (getline(inputFile, instructions)) {
            ...
            switch(device) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    // Input/Output Device Execution
                    _iodevice->parseInstructions(instructions);
                    break;
                default:
